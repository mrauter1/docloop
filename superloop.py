...
4. End the last non-empty line of stdout with exactly one tag:
...
- Ask `<question>...</question>` only when missing product intent makes safe verification impossible, and include best suggestion/supposition.
...
3. Label each finding as `blocking` or `non-blocking`.
4. End the last non-empty line with exactly one promise tag:
<promise>COMPLETE</promise> OR <promise>INCOMPLETE</promise> OR <promise>BLOCKED</promise>
...
- Do not return `INCOMPLETE` if you have no blocking findings.
- Ask `<question>...</question>` only for missing product intent, and include best suggestion/supposition.
