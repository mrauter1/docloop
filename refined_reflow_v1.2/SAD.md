...
* Any path that Reflow resolves from workflow or config data MUST remain inside the workspace after lexical normalization and symlink resolution.
...
* `entry` MUST be a declared step name and MUST NOT begin with `@`.
...
* Each `<question>` body MUST be non-empty after trimming surrounding whitespace.
* Leading text before the final `<questions>` block is allowed.
* Only trailing whitespace MAY appear after the closing `</questions>` tag.
...
* If `allow_write` is absent, non-Reflow workspace writes are allowed unless they match `forbid_write`.
...
* Path matching for policy enforcement uses normalized repo-relative paths.
* For a rename, both the source path and the destination path are evaluated against policy.
* If a changed path resolves outside the workspace after symlink resolution, Reflow MUST treat that change as a policy violation.
...
* `step_loops` MUST contain every declared step name with a non-negative integer value.
...
* The initial `run.json` MUST set every declared step loop counter to `0`.
...
* Reconciling a reserved iteration means marking the reserved iteration `interrupted`, setting `ended_at` if it is missing, appending an `iteration_finished` history record with `status: interrupted`, leaving any existing `stdout.txt`, `stderr.txt`, and `final.txt` content unchanged, and then starting a fresh next loop.
...
* `input_auto_failed`: `step`, `loop`, `auto_round`, `reason`
...
* `history.jsonl` MUST contain exactly one UTF-8 JSON object per line.
* History records MUST be appended in controller event order and MUST NOT be rewritten or deleted.
* History timestamps for a single run MUST be non-decreasing.
...
* Full-auto answer passes do not create step iteration directories in v1.2.
...
* `command_argv` is the exact argv vector passed to process creation; for `shell` steps it MUST be `["/bin/sh", "-lc", "<cmd>"]`.
...
* The header line MUST use the exact shape `## <timestamp> | <step> | loop <n> | <mode>`.
* Each entry MUST contain exactly one `Qn:` and one `An:` block for each pending question, numbered from `1` in order.
* Multi-line question and answer content MUST preserve line order; continuation lines MUST be indented by two spaces.
...
* Reflow MUST NOT inline the contents of `.reflow/context.md` or `operator_inputs.md`; the request may reference their paths, but those files remain repository-resident memory.
...
A valid full-auto answer response is a final `<answers>` block containing one `<answer>` element per pending question:
...
* The `<answers>` block MUST be the final non-empty structured control block in the full-auto provider output.
* The block MUST contain exactly one `<answer>` element per pending question, in order.
* Each `<answer>` body MAY span multiple lines and MUST be preserved verbatim except for trimming surrounding trailing whitespace.
* Leading text before the final `<answers>` block is allowed.
* Only trailing whitespace MAY appear after the closing `</answers>` tag.
* If the output contains `<answer>` tags outside a valid final `<answers>` block, the answer pass is a protocol error.
...
2. If `pending_input.auto_round >= max_auto_rounds`, Reflow MUST enter `awaiting_input` instead of launching another auto-answer pass.
3. Otherwise, Reflow MUST increment `pending_input.auto_round`, persist it, and invoke a fresh provider pass using the workflow `full_auto_instructions` file, or the built-in default if none is configured.
4. The full-auto answer pass MUST use the same resolved provider profile as the step that emitted the questions.
5. The full-auto answer request MUST include the pending questions, workspace context, and the exact `<answers>` contract from Section 14.4.
6. The full-auto answer pass MAY mention that repository inspection and web search are allowed when the provider and operator configuration permit them.
7. A full-auto answer pass is not a workflow step iteration. It MUST NOT increment `step_loops`, MUST NOT increment `cycle_count`, and MUST NOT create a step iteration directory.
8. If the answer pass cannot be started, exits nonzero, times out, emits an invalid `<answers>` block, or returns the wrong number of answers, Reflow MUST append `input_auto_failed`, leave `pending_input` intact, set the run to `awaiting_input`, and follow the waiting-state rules from Section 14.3.
9. On success, Reflow MUST append the resulting answers to `operator_inputs.md` with `mode: auto`, clear `pending_input`, append `input_resolved`, and start a fresh iteration of the same step.
...
9. apply required-file checks when policy requires them and the accepted transition is neither `@retry` nor `@blocked`
10. if the accepted transition is `@retry`, rerun the same step if loops remain, else fail with `max_loops_exceeded`
11. if the step counts toward cycles and the accepted transition is not `@retry`, increment `cycle_count`
12. if a workflow `max_cycles` cap is configured and `cycle_count > max_cycles`, fail the run with `max_cycles_exceeded`
13. if the accepted transition is another step, update `current_step` and continue
14. if the accepted transition is `@done`, mark the run `completed`
15. if the accepted transition is `@blocked`, mark the run `blocked`
...
* `step_failed`: provider exited nonzero after startup, provider timed out, malformed control output, required files missing, or a write-policy violation occurred.
...
* A shell-step nonzero exit is routed through `on_failure` and is not, by itself, a terminal `failure_type`.
* A failed full-auto answer pass does not set `failure_type`; it returns the run to `awaiting_input`.
...
* `list` MUST enumerate runs from `.reflow/runs/` only and include at least `run_id`, `workflow`, `status`, and `updated_at` for each run.
* `list` output MUST be sorted by `started_at` descending.
* v1.2 does not define a machine-stable output format for `status` or `list`.
...
* For Claude, Reflow MUST pass the rendered request as the `-p` prompt argument.
